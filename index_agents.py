    ...
    mapping = {
        "mappings": {
            "properties": {
                ...
                "manifest": {"type": "object", "enabled": False} # The manifest is returned but not indexed deeply
            }
        }
    }
...
def index_agents_from_file(es_client: Elasticsearch, file_path: str):
    ...
    try:
        ...
        for idx, agent_dict in enumerate(agents_data):
            try:
                ...
                logger.info(f"Indexed agent '{agent.agent_id}' (Result: {res['result']})")
...
def main():
    ...
    try:
        # Check connection
        if not es.ping():
            logger.warning("Could not connect to Elasticsearch at http://localhost:9200. Is it running?")
